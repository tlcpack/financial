...
demo = 'd8a1759a992a7461e2cefcb3b2fb5ee4'

def historical_prices(stock):
	historical_prices = requests.get(f'https://financialmodelingprep.com/api/v3/historical-price-full/{stock}?apikey={demo}').json()
...
def income_statement(stock):
 	...
 	IS = requests.get(f'https://financialmodelingprep.com/api/v3/income-statement/{stock}?period=quarter&limit={number_qts}&apikey={demo}').json()
...
def profile(stock):
	profile = requests.get(f'https://financialmodelingprep.com/api/v3/profile/{stock}?apikey={demo}').json()
	profile = profile[0]
	print(profile)
...
def balance_sheet(stock):
	number_qts = input('number_qts?').strip()
	save_to_csv = input('save_to_csv? y or n').strip()
	BS = requests.get(f'https://financialmodelingprep.com/api/v3/balance-sheet-statement/{stock}?period=quarter&limit={number_qts}&apikey={demo}').json()
...
def valuation_dcf(stock):
	valuation_dcf = requests.get(f'https://financialmodelingprep.com/api/v3/discounted-cash-flow/{stock}?apikey={demo}').json()
...
def dividends(stock):
	...
	dividends = requests.get(f'https://financialmodelingprep.com/api/v3/historical-price-full/stock_dividend/{stock}?apikey={demo}').json()
